...
<div class="user-management">
    ...
    <form action="pages/penjualan_save.php" method="POST">
                    ...
                    <select name="kode" id="kode_barang" class="form-select" required onchange="cekHarga()">
                        ...
                        <?php 
                            ...
                            $sql = "SELECT d.kode, d.nama, COALESCE(h.harga_jual, 0) as harga_jual, COALESCE(s.jumlah, 0) as jumlah 
                                    ...
                                    WHERE d.hapus = 0 ORDER BY d.nama ASC";
                            $q = $conn->query($sql);
                            if ($q) {
                                while($r = $q->fetch_assoc()){
                                    echo "<option value='".$r['kode']."' data-harga='".$r['harga_jual']."' data-stok='".$r['jumlah']."'>".$r['nama']." (Stok: ".$r['jumlah'].")</option>";
                                }
                            }
                        ?>
                    </select>
        ...
        <div class="d-flex justify-content-end gap-2">
            <a href="main.php?p=detail_jual_manage" class="btn btn-secondary">Batal</a>
